...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		requirejs: {
			compile: {
				options: {
					...
					out: "<%=pkg.folders.build %>/js/main.js",
					optimize: "uglify2",
					paths: {
			...
			development: {
				options: {
					modifyVars: {
						...
						"ngstart-images-path": '"../images/"'
					}
				},
				files: {
					"<%=pkg.folders.build%>css/style.css": "<%=pkg.folders.wwwRoot%>less/style.less"
				}
			},
			production: {
				options: {
		...
		autoprefixer: {
			...
			development: {
				src: '<%=pkg.folders.build%>/css/style.css',
				dest: '<%=pkg.folders.build%>/css/style.css'
			},
			production: {
				options: {
					cascade: false
		...
		compress: {
			tgz: {
				options: {
					mode: "tgz",
					archive: "<%= pkg.folders.build + pkg.version + '.tar.gz'%>"
				},
				expand: true,
				src:  ['**/*', '**/.*'],
				dest: '<%= pkg.version %>/',
